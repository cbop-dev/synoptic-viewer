...
/**
 * @type {Object<string,string>} bibleRefReverseLookupHash
 */
export var bibleRefReverseLookupHash = {};


/** @type {Record<string, string[]>} */
export var bookAbbrevMap =
{
    /*OT:*/
    'Gen': ['Genesis', 'Ge'],
    'Exod': ['Exodus'],
...
for (const [key, aliases] of Object.entries(bookAbbrevMap)) {
    ...
    const toCheck = [...aliases, key];
    for (const name of toCheck) {
        if (name.includes(' ')) {
            expanded.add(name.replaceAll(' ', '_'));
            expanded.add(name.replaceAll(' ', ''));
        }
    }
    bookAbbrevMap[key] = Array.from(expanded);
}
...
/**
 * 
 * @param {{book:string,chap:string,v:string}} bcv 
 * @returns {string}
 */
export function bookChapVerseToString(bcv){
    let ret = bcv.book ? (bcv.book) + ' ': '';
...
/**
 * used as the compare function in sort() to sort books by their order in the bible, per the list above.
 * @param {string} b1 
 * @param {string} b2 
 * @param {BookDict|null} booksDict -- the book dictionary to use for book sorting. If null, use the default book ordering (defined above). */
export function sortBooksFunc(b1,b2,booksDict=null){
    return getBookOrder(b1,booksDict) - getBookOrder(b2,booksDict);
}
...
export function expandRefs(refString, splitVerseRanges = true) {
    ...
    for (const ref of refString.split(";").filter((s) => s)) {
        if (ref.length) {
            ...
            if (bookCv.book) { //got no chap, thus just a (whole) book
                if (!bookCv.chap) { //book only!
                    refArray.push(bookCv.book);
                }
                //got a chapter, check for verses
                else if (bookCv.v?.includes(',')) {
                    for (const vv of bookCv.v.split(',')) {
                        if (vv.includes('-') && splitVerseRanges) {
                            mylog("expandRefs(), splitting verses!  ")
                ...
                else if (bookCv.v?.includes('-') && splitVerseRanges) { //got book, but got verse range
                    for (const v of mathUtils.createNumArrayFromStringListRange(bookCv.v)) {
                        refArray.push(bookCv.book + " " + bookCv.chap + (v ? ":" + v : ''));
                    }
                }
                else if (bookCv.chap?.includes('-') && !bookCv.v){ //got chapter range, but not verses (as is right and just)
                    for (const c of mathUtils.createNumArrayFromStringListRange(bookCv.chap)) {
                        refArray.push(bookCv.book + " " + c);
                    }
                }
                else {
 ...
 * @param {BookDict|null} booksDict -- the book dictionary to use for book name lookups and sorting. If null, use the default book ordering (defined above). * @returns -- a string with the bible references grouped and  combined by book and chapter, such that larger sections swallow smaller ones.
 *         e.g., ["Gen", "Gen 1"] => "Gen", but ["Gen 1", "Gen 2:4", "Exod 3:2"] => "Gen 1; 2:4; Ex 3:2".
 */
//NB: TODO: fix so it works with "1 Thess","2 Thess", etc.
export function combineRefs(refArray,booksDict=null) {
    ...
    const combined = Object.entries(refList)
        .sort(([b1,chapList1], [b2,chapList2]) => sortBooksFunc(b1, b2,booksDict))
        .map(([b, chapList]) => {
        
        let chaps = '';
        if (chapList != 'all' && Object.keys(chapList).length > 0){

            //joinging books and verses. How can we join whole consecutive chapters?
            //join consecutive chapters!
            chaps += joinInRanges(Object.entries(chapList)
            .filter(([c,vv])=> vv =='all' || vv.length == 0).map(([c,vv])=> c),"; ");

            
           const chaps2 = 
                Object.entries(chapList).filter(([c,vv])=> vv !='all' && vv.length > 0).map(
                //got verses!
                ([c, vv]) =>{
                    const vvString = String(vv);
                  //mylog(`joining vv: '${vv}'; type: '${typeof String(vv)}'`, true);
                  let mapped = c + ":" + joinInRanges(vvString.split(','))//.sort((a, b) => Number(a) - Number(b)))
                  return mapped;
                }).join("; ");
            chaps = [chaps, chaps2].filter(_=>_).join("; ");
        }
        else{

        }
        return b+ (chaps ? " " + chaps : '');
...
/**
 * 
 * @param {string[]} chapterVerseRefArray -- an array chapter (with, possibly verses, such as either "6" or "6:1-2")
 * @returns {string[]}
 */
export function joinChapters(chapterVerseRefArray){
            ...
            "\n").filter((s) => s.trim()).map((item) => item.trim()).forEach(l => {
                if (l.trim().length > 0) {
                    ...
                    vv.split(",").map(item => item.trim()).forEach(v => {
                        ...
                        else {/*do nothing, verse already listed*/
 ...
 * 
 * @param {string} lookup 
 * @param {Object} booksDict 
 * @returns {string}
 */
export function matchBookName(lookup,booksDict=null) {
    var book = ''
    if(booksDict && booksDict.lookupBookAbbrev){
        book = booksDict.lookupBookAbbrev(lookup);
    }
    else if (lookup.length > 0 && bibleRefReverseLookupHash[lookup.toLowerCase()]) {
...
export function buildbibleRefReverseLookupHash() {
 ...
 * @param {Object<number,{name:string,abbrev:string, syn:string[],words:number,chapters:number,lemmas:number}>|null} [booksDict=null] 
 * @returns 
 */
export function getBookOrder(bookName,booksDict=null) {
    ...
    */
    ...
    if (abbrev){
        if (booksDict){
            found = Object.values(booksDict).map((b,i)=>[b.syn,i]).find(([syn,i])=>syn.includes(abbrev));
            if(found && found.length > 1) order = found[1]
        }
        else {
             found = matchBookName(abbrev);
             if (found) order = Object.keys(bookAbbrevMap).indexOf(abbrev);
        }
    }
    return order;
 ...
 * @returns {string} the refString but with spaces added where they should be. Thus "2Kg 2:14"->"2 Kg 2:14" and "1_Corinthians 1:1"->"1 Corinthians 1:1".
 */
export function standaradizeBibleRef(refString) {
    return refString.trim().replace(/^([0-9])([A-Za-z])/g, '$1 $2').replace(/_/g, ' ').replace(/ +/g, ' ');
 ...
 */
export function filterOutInvalidBooks(refString,booksDict=null){

    return combineRefs(expandRefs(refString)
        .filter((ref)=>matchBookName(getBookChapVerseFromRef(ref)?.book || '',booksDict)));
