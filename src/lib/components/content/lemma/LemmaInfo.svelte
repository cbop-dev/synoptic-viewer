...
<script>
	...
	let theBookName = $derived.by(() => {
		if (bookID) return untrack(() => tfServer.getBookSyn(bookID));
		else return '';
	});
	let theBookAbbrev = $derived.by(() => {
		return bookID ? untrack(() => tfServer.getBookAbbrev(theBookName)) : '';
	});
	...
	let lemmaBookCountsChartData = {
		...
		labels: Object.keys(lemmaInfo.stats.bookStats).map(
			(id) => tfServer.booksDict[Number(id)].abbrev
		)
	};

	let lemmaBookCountChartOptionIndex = $state(0);
	let lemmaBookCountChartOptions = ['Data Table', 'Lemma Count', 'Frequency'];

	let lemmaBookFreqChartData =
		lemmaInfo.stats.bookStats && Object.keys(lemmaInfo.stats.bookStats).length
			? {
		...
		!lemmaBookCountsChartData || !lemmaBookFreqChartData
			...
			: {
					data: lemmaBookCountsChartData.nums.map((count, index) => [
						...
						lemmaInfo.stats.totalFreq
							? MathUtils.floatRound(
									lemmaBookFreqChartData.nums[index] / lemmaInfo.stats.totalFreq,
									3
								)
							: 0
					]),
...
<div class="items-center text-center">
	...
	<div class="block text-center m-0 p-0 mt-1 self-center">
		<ButtonSelect bind:selected={showReferences}
			><Icon svg={BookSvg} />See all {lemmaInfo.stats.count} reference(s)</ButtonSelect
		>
		<ButtonSelect bind:selected={enableTextView}
			><Icon svg={BookOpenSvg} />View the Texts</ButtonSelect
		>
		...
		{#if false && bookID > 0}
			<ButtonSelect
				buttonText="See {lemmaInfo.stats.bookStats[bookID].count} reference(s) in section."
		...
		<div class="rounded bg-slate-200 p-1 shadow">
			<h2 class="font-bold">
				<span class="underline"
					>All {lemmaInfo.stats.count} NT Occurrences of {lemmaInfo.lemma}</span
				>
				<ButtonSelect
					buttonStyle="btn btn-xs m-1 p-1"
					buttonText="See Texts"
					bind:selected={enableTextView}
				/>
			</h2>
			<div class="rounded-2xl bg-slate-100/80 p-2 shadow">
				{theRefs}
				<CopyText copyText={theRefs} />
			</div>
		</div>
...
{/each}-->
	...
	{#if showStats}
		...
		<span
			...
			>"What is the wisest? Number. The second is what gives things their names." --Pythagorus</span
		>
		<span
			...
			>"There are three kinds of lies: lies, d*mned lies, and statistics." -- Mark Twain</span
		>

		<Tabs
			headings={tabs}
			bind:selectedTabIndex={selectedTab}
				...
				{#if sections.length > 0}
					...
					<div class="stats shadow inline-block">
						<div class="stat">
							...
							<div class="stat-desc">
								Total instances of {lemmaInfo.lemma} (minus {theBookAbbrev}) in {tfServer.name}
							</div>
						</div>
					</div>
				{/if}
				...
				{#if lemmaInfo.stats.bookStats[bookID].freq.book}
					<div class="stats shadow inline-block">
						<div class="stat">
							...
							<div class="stat-desc">
								Frequency of {lemmaInfo.lemma} in {theBookAbbrev} per 1,000 words
							</div>
						</div>
					</div>
				{/if}
				...
				<div class="stats shadow inline-block">
					<div class="stat">
						...
						<div class="stat-value">
							{(
								(100 * lemmaInfo.stats.bookStats[bookID].lexCounts.book) /
								lemmaInfo.stats.count
							).toFixed(1)}%
						</div>
						<div class="stat-desc">Frequency of {lemmaInfo.lemma} per 1,000 words in NT</div>
					</div>
			...
			{#key bookID}
					...
					<div class="stats shadow inline-block">
						...
						<div class="stat">
							<div class="stat-title">
								{lemmaInfo.stats.bookStats[bookID].lexCounts.book} of {lemmaInfo.stats.count}
							</div>
							<PieChart
								pieData={{
									labels: [theBookAbbrev, 'Rest of NT'],
									nums: [
										lemmaInfo.stats.bookStats[bookID].lexCounts.book,
										lemmaInfo.stats.bookStats[bookID].lexCounts.restNT
									]
								}}
							/>
							...
							<div class="stat-desc">
								{theBookAbbrev}'s share of NT's total use of {lemmaInfo.lemma}
							</div>
						</div>
					</div>
				{/if}
				...
				{#if lemmaInfo.stats.bookStats[bookID].freqRatio && sections}
					<div class="stats shadow inline-block">
						...
						<div class="stat">
							<div class="stat-title">
								How much more/less frequently does {theBookAbbrev} use {lemmaInfo.lemma} than the rest
								of the NT?
							</div>
							{#if lemmaInfo.stats.count == lemmaInfo.stats.bookStats[bookID].lexCounts.book}
								<div class="stat-value">
									{#if lemmaInfo.stats.count == 1}Hapax!{:else}No contest!{/if}
								</div>
								<div class="stat-desc">
									<span class="italic"
										>{theBookName} is the only NT book to use {lemmaInfo.lemma}{#if lemmaInfo.stats.count == 1},
											and he does so only once{/if}!</span
									>
								</div>
							{:else}
								<BarChart
									barData={{
										labels: [theBookAbbrev + ' freq.', 'Rest of NT freq.'],
										nums: [
											lemmaInfo.stats.bookStats[bookID].freq.book.toFixed(3),
											MathUtils.floatRound(lemmaInfo.stats.bookStats[bookID].freq.restNT, 3)
										]
									}}
								/>
								<div class="stat-value">
									{lemmaInfo.stats.bookStats[bookID].freqRatio.toFixed(3)}
								</div>
								...
								<div class="stat-desc font-bold italic">
									...
									{:else if lemmaInfo.stats.bookStats[bookID].freqRatio > 0.8}too close to call{:else}the
										NT{/if}!
								</div>
							{/if}
						</div>
					...
					{#if lemmaBookCountsChartData}
						...
						<div class="mt-1 rounded-2xl shadow-2xl p-2 bg-white">
							<select
								...
								class="inline-block align-top text-center self-center"
							>
							...
							{#if lemmaBookCountChartOptionIndex == 1}
								<div id="lemma-info-book-counts-chart">
									<BarChart barData={lemmaBookCountsChartData} horizontal={true} />
								</div>
							{:else if lemmaBookCountChartOptionIndex == 2}
								<div id="lemma-info-nt-book-counts-chart">
									{#key lemmaBookCountChartOptionIndex}
										<BarChart barData={lemmaBookFreqChartData} horizontal={true} />
									{/key}
								</div>
