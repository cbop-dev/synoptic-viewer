<script>
	...
	import {
		getServer,
		sbl as sblServer,
		n1904 as n1904Server,
		vulgate as vulgateServer,
		web as webcServer
	} from '$lib/tf/tfServer.js';
	 ...
	 */
	let { options = new SynopsisOptions3() } = $props();
	...
	let currentServerName = $state(
		myServers.lookup(options.request.nt) ? options.request.nt : myServers.list[0].abbrev
	);
	 ...
	 */
	let tfServer = $derived(
		myServers.lookup(currentServerName)
			? myServers.lookup(currentServerName)
			: myServers.list[0].server
	);
	//let tfServer=$state(getServer());
	// let { data=null } = $props();

	const panes = [
		{ name: 'Gospels', short: 'Gos', comp: NtSynopsisPanel },

		{ name: 'Custom', short: 'Cust', comp: CustomParallelViewer }
	];
...
{#snippet appSummary(heading = true, headingTag = 'h1')}
	...
	<SiteInfo />
{/snippet}
<svelte:window bind:scrollY={y} bind:innerHeight={windowHeight} {onkeydown} />
<div
	id="app-container"
	style="--codex-img: url('{CodexAmatt1Wide}'); --bg-app: {bgApp}; --bg-content: {bgContent};"
>
	<div id="main-wrap" style="--bgCodex: url('{CodexAmatt1Wide}'); --bgLinen: url('{LinenPng}');">
		...
		<div id="main-content" class="relative" bind:clientHeight={contentHeight}>
			<div
				id="top-tab-bar"
				class="block top-1 left-0 z-100 w-full text-right fixed"
				bind:clientHeight={headerHeight}
			>
				<div
					id="tabs"
					role="tablist"
					class="inline-block tabs tabs-lifted self-baseline text-baseline"
				>
					{#each panes as pane, index}
						<a
							role="tab"
							class="tab {selectedPane == index ? 'tab-active' : ''} "
							tabindex={index}
							onclick={() => {
								selectedPane = index;
							}}
						>
							<span class="sm:inline hidden">{pane.name}</span><span class="inline sm:hidden"
								>{pane.short}</span
							></a
						>
					{/each}
				...
				<div class="inline-block mr-1 mt-0.5 mb-0.5">
					<ButtonSelect
						...
						bind:selected={showInfoModal}
						tooltip="Show site info"
						tooltipbottom={true}
					/>
					<ButtonSelect
						buttonStyle="btn btn-xs rounded   btn-square  p-0.5 m-0"
						bind:selected={enableKeys}
						buttonText="k"
						tooltip="Enable/disable hotkeys"
						tooltipbottom={true}
					/>
					<ButtonSelect
						buttonText={`☰ ${currentServerName}`}
						buttonStyle="btn btn-xs  btn-round btn-ghost   p-0.5 m-0"
						bind:selected={showNTselect}
						tooltip="Select NT version"
						tooltipbottom={true}
					/>
				</div>
				...
				{#if showNTselect}
					<div
						id="version-select-panel"
						class="blockitems-center absolute right-0 m-2 p-5 rounded-2xl outline-2"
					>
						<label for="ntversion" class="hidden md:inline m-0 p-0 text-sm">NT version:</label>
						<select
							name="ntversion"
							class="m-2 ml-1 mr-1 p-1 text-sm"
							bind:value={currentServerName}
						>
						...
						<span class="italic text-sm hidden md:inline-block"
							>Takes effect after submitting a new query.</span
						>
						<Button
							tooltip="Close"
					...
					<div id="pane-{pane.name}" class={index == selectedPane ? 'block' : 'hidden'}>
						{#if options.request.tab == index}
							<pane.comp
								...
								live={index == selectedPane}
								{tfServer}
								keyevent={index == selectedPane ? keyEvents[index] : null}
								--bg-app={bgApp}
	...
	<div class="fixed bottom-0 right-0 z-100 mr-1 mb-1">
		<!--up//downarros-->
		...
		{#if y > 100}
			<div class="block">
				<a
					href="#"
					class="inline bg-white/60 border-slate-400/80 border-1 p-0.5 rounded-lg"
					title="Top"><ArrowTop height={20} width={20} /></a
				>
			</div>
		{/if}
...
<style>
	...
	#maiddn-bg {
		...
		width: 100%;
		height: 100%;
		filter: opacity(var(--bg-opacity)) saturate(var(--bg-saturation))
