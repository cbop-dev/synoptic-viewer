<script>
	...
	let {
		...
		useGospels = true,
		conditions = { hasApparatus: true, hasLexicalInfo: true, hasPhraseComparisionCondition: true }
	} = $props();
...
{#if showResultsButtons}
	<ButtonSelect
		buttonText="☰"
		buttonStyle="btn  btn-ghost btn-circle btn-sm p-0 pr-1 m-0 pl-1 ml-0.5"
...
<svelte:element this={headingTag} class={[titleClasses]}>
	<a data-sveltekit-reload href="/"
		...
		{:else}
			...
			<span class={['sm:hidden inline text-nowrap', extraTitleclasses]}
				>{shorttitle ? shorttitle : mediumtitle ? mediumtitle : title}</span
			>
			<span class={['sm:inline md:hidden text-nowrap hidden', extraTitleclasses]}
				>{mediumtitle ? mediumtitle : shorttitle ? shorttitle : title}</span
			>
...
<ul class="menu menu-horizontal w-auto">
		...
		{#each hotkeys.getNavButtonKeys() as hk}
			{#if !hk.condition?.property || (Object.keys(conditions).includes(hk.condition.property) && conditions[hk.condition.property] == hk.condition.value)}
				<li class={[showResultsButtons ? 'sm:list-item' : '', 'hidden']}>
					<ButtonSelect
						buttonText={hk.key}
						buttonStyle={'btn btn-xs btn-circle btn-ghost p-0 ml-0.5 text-md ' +
							(hk.navKeyClasses ? hk.navKeyClasses : '')}
						bind:selected={options.viewOptions[hk.optionName]}
						tooltipbottom
						tooltip={hk.description ? hk.description : hk.name}
					/>
				</li>
			{/if}
		{/each}
	...
	{#if useGospels && (options.viewOptions.similarPhrases || options.viewOptions.exactPhrases)}
		<li class="sm:list-item">
			<ButtonSelect
				buttonText="Colors"
				buttonStyle="btn btn-xs  btn-ghost p-0 ml-0.5 text-md "
				bind:selected={showColorsModal}
				tooltipbottom
				tooltip="Show Matching Colors Key"
			/>
		</li>
	{/if}
</ul>
