<script>
	...
	const requestModes = [{ name: 'Single Group' }, { name: 'Batch' }];

	let selectedRequestModeIndex = $state(
		myOptions.request.mode < requestModes.length ? myOptions.request.mode : 0
	);
	 ...
	 */
	let refAreaInputs = $state(
		myOptions.request.columns && myOptions.request.columns.length
			? myOptions.request.columns
			: ['Matt 1:1', 'Mark 1:1']
	);
	...
	let selectedGreekPalette = $derived(
		ColorUtils.myColorPalette(selectedLexes.length + myOptions.viewOptions.greekStrings.length)
	);
	...
	let customGreekClasses = $derived.by(() => {
		// (greek expression)->css color (e.g., "#eee")
	...
	let unselectedLexes = $derived.by(() => {
		if (response && response.lexemes) {
			return Object.values(response.lexemes)
	...
	$effect(() => {
		customGreekInputText = GreekUtils.onlyPlainGreek(
...
{#snippet titleButtons(hideLookup = false, hideHelp = false)}
	<ul class="bg-white menu menu-horizontal w-auto">
		...
		{#each hotkeys.getNavButtonKeys() as hk}
			<li
				class={[
					!myOptions.viewOptions.menuOpen && dataReady ? 'sm:list-item' : '',
					'hidden',
					'm-0',
					'p-0'
				]}
			>
				<ButtonSelect
					buttonText={hk.key}
					buttonStyle="btn btn-xs btn-circle btn-ghost p-0 m-0 sm:ml-0.5 "
					bind:selected={myOptions.viewOptions[hk.optionName]}
					tooltip={hk.name}
					tooltipbottom
				/>
			</li>
		...
		{#if currentServer.abbrev == SblGntServer.abbrev}
			...
			>
				<label class="label" for="hide-app-check2">
					...
					/>Hide appar{#if !short}atus{:else}.{/if}</label
				>
			</svelte:element>
		{/if}
		<svelte:element this={theTag} class={classes}
	...
	<div class="navbar bg-base-100 text-left sm:text-center shadow-sm pb-0 mb-0 sm:mb-1 sm:pb-1">
		<div class="text-left sm:navbar-center sm:self-center sm:w-full sm:m-auto">
			<div class="text-left sm:text-center sm:self-center w-full border-0">
				<div id="title-panel">
					<TitleNavbar
						title="Custom Greek NT Synopsis"
						mediumtitle="Custom NT Synopsis"
	...
	<div class="max-w-full block text-center">
			...
			{#if selectedLexes.length}
				...
				{#each selectedLexes as lex, index}
					<Button
						...
						buttonColors={getColorOfLex(lex)}
		...
		<div class={selectedWordTabIndex == 1 ? 'block' : 'hidden'}>
			...
			<i
				...
				will αυτοματως convert to Greek.</i
			><br />
			<input
				type="text"
				size="15"
	...
	<div class="m-auto items-center text-center">
		<div class="inline-block text-left">
			...
			<ol class="list-decimal ml-9">
				<li>
					<span class="bold">Single group mode:</span> Enter a group of texts in each column (add/subtract
					columns by clicking the +/- buttons). Seperate references within a column with a colon or newline.
				</li>
				<li>
					<span class="bold">Batch mode (aka "Awesome Sauce"):</span>Enter several groups of rows,
					one group per row. Separate columns with a "pipe" (|) symbol. Separate references within a
					single column by a colon.
				</li>
			</ol>
			Press "Lookup!" when you're ready. Then hold on to your exegetical seatbelts and enjoy the NT synopsis
			ride...
		</div>
		<hr class="m-5" />
		...
		<div class=" m-auto inline-block text-left">
			<ul class="list-disc ml-9">
				...
				<li>
					Enabling <b>"Auto Highlight"</b> will toggle
					<span class="bg-cyan-500 text-white">highlighting</span> of all instances of the lexeme.
				</li>
				<li>
					Enabling <b>"Similar"</b> will underline and highlight "lexically-identical" phrases that are
					common between at least two columns. "Lexically identical" here refers to those phrases which
					have the same lexemes in the same sequence even if they differ morphologically. This crosses
					verse boundaries.
				</li>
				<li>
